...
from app.tests.users.test_users import (
    TestUsers,
)  # Ensure this import exists and points to your TestUsers setup
...
class TestMaintenanceRequest:
    ...
    @pytest.mark.dependency(
        depends=["TestUsers::test_create_user"], name="create_maintenance_request"
    )
    async def test_create_maintenance_request(self, client: AsyncClient):
        ...
        ), f"Failed to create maintenance request: {response.text}"
    ...
    async def test_update_maintenance_request(self, client: AsyncClient):
        ...
        assert (
            response.status_code == 200
        ), f"Failed to update maintenance request: {response.text}"
    ...
    async def test_delete_maintenance_request(self, client: AsyncClient):
        ...
        ), f"Failed to delete maintenance request: {response.text}"

        # Verify the maintenance request is deleted
        response = await client.get(f"/maintenance-request/{maintenance_request_id}")
        assert (
            response.status_code == 404
        ), f"Maintenance request not properly deleted: {response.text}"
