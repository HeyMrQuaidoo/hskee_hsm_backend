...
class TestContractType:
    ...
    async def test_create_contract_type(self, client: AsyncClient):
        ...
        check_response = await client.get("/contract-type/", params={"contract_type_name": "lease"})
        ...
        if check_response.status_code == 200 and check_response.json().get("data"):
            ...
            if contract_type_id:
                delete_response = await client.delete(f"/contract-type/{contract_type_id}")
                assert delete_response.status_code in [200, 204], f"Failed to delete existing contract type: {delete_response.text}"
        ...
        assert response.status_code == 201, f"Failed to create contract type: {response.text}"
