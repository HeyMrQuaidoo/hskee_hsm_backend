...
class TestPropertyAssignment:
    ...
    @pytest.mark.dependency(depends=["TestProperties::test_create_property", "TestUsers::test_create_user"], name="create_property_assignment")
    async def test_create_property_assignment(self, client: AsyncClient):
        ...
        response = await client.post(
            "/assign-properties/",
            json={
                "property_unit_assoc_id": self.default_property["property_unit_assoc_id"],
    ...
    async def test_get_all_property_assignments(self, client: AsyncClient):
        response = await client.get("/assign-properties/", params={"limit": 10, "offset": 0})
        assert response.status_code == 200
    ...
    @pytest.mark.asyncio(loop_scope="session")
    @pytest.mark.dependency(depends=["delete_property_assignment_by_id"], name="delete_property_by_id")
