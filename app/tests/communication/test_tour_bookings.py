...
class TestTourBookings:
    ...
    @pytest.mark.asyncio(loop_scope="session")
    @pytest.mark.dependency(
        depends=["TestTourBookings::create_tour_booking"],
        name="TestTourBookings::create_additional_tours",
    )
    async def test_create_additional_tours(self, client: AsyncClient):
        ...
        for tour_data in tour_data_list:
            ...
            ), f"Additional tour creation failed: {response.text}"

    @pytest.mark.asyncio(loop_scope="session")
    @pytest.mark.dependency(
        depends=["TestTourBookings::create_additional_tours"],
    ...
    @pytest.mark.dependency(
        depends=["TestTourBookings::create_tour_booking"],
        name="TestTourBookings::get_tour_booking_by_id",
    )
    ...
    @pytest.mark.asyncio(loop_scope="session")
    @pytest.mark.dependency(
        depends=["TestTourBookings::get_tour_booking_by_id"],
        name="TestTourBookings::update_tour_booking_by_id",
    )
    ...
    @pytest.mark.dependency(
        depends=["TestTourBookings::create_tour_booking"],
        name="TestTourBookings::filter_tours",
    )
    async def test_filter_tours(self, client: AsyncClient):
        ...
        ), f"Failed to filter tours by status: {response.text}"
        data = response.json()["data"]
        assert all(
            tour["status"] == "incoming" for tour in data
        ), "Filtering by status failed"

        # Filter by tour_type
        response = await client.get("/tour/", params={"tour_type": "video_chat"})
        assert (
            response.status_code == 200
        ), f"Failed to filter tours by tour_type: {response.text}"
        data = response.json()["data"]
        assert all(
            tour["tour_type"] == "video_chat" for tour in data
        ), "Filtering by tour_type failed"

        # Filter by date range
        start_date = (datetime.now() - timedelta(days=2)).isoformat() + "Z"
        end_date = (datetime.now() + timedelta(days=2)).isoformat() + "Z"
        response = await client.get(
            "/tour/", params={"date_gte": start_date, "date_lte": end_date}
        )
        assert (
            response.status_code == 200
        ), f"Failed to filter tours by date range: {response.text}"
        data = response.json()["data"]
        ...
        assert (
            response.status_code == 200
        ), f"Failed to filter tours by user_id: {response.text}"
        data = response.json()["data"]
        assert all(
            tour["user_id"] == user_id for tour in data
        ...
        assert (
            response.status_code == 200
        ), f"Failed to filter tours by email: {response.text}"
        data = response.json()["data"]
        assert all(
            "janesmith@example.com" in tour["email"] for tour in data
        ), "Filtering by email failed"

        # Filter by name
        response = await client.get("/tour/", params={"name": "Alice"})
        assert (
            response.status_code == 200
        ), f"Failed to filter tours by name: {response.text}"
        data = response.json()["data"]
        ...
        assert (
            response.status_code == 200
        ), f"Failed to filter tours by phone_number: {response.text}"
        data = response.json()["data"]
        assert all(
            "555" in tour["phone_number"] for tour in data
        ), "Filtering by phone_number failed"

    @pytest.mark.asyncio(loop_scope="session")
    @pytest.mark.dependency(
        depends=["TestTourBookings::update_tour_booking_by_id"],
        name="TestTourBookings::delete_tour_booking_by_id",
    )
    async def test_delete_tour_booking(self, client: AsyncClient):
        ...
        ), f"Failed to get tour by ID after deletion: {response.text}"
