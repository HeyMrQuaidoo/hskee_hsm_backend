...
class TestFavoriteProperties:
    ...
    async def test_create_favorite_property(self, client: AsyncClient):
        ...
        ), f"Favorite property creation failed: {response.text}"
        ...
        self.property_ids.append(
            TestProperties.default_property.get("property_unit_assoc_id")
        )
    ...
    async def test_create_additional_favorites(self, client: AsyncClient):
        ...
        for i in range(2):
            ...
            ), f"Property creation failed: {property_response.text}"
    ...
    async def test_get_all_favorite_properties(self, client: AsyncClient):
        response = await client.get(
            "/favorite-properties/", params={"limit": 10, "offset": 0}
        )
        assert (
            response.status_code == 200
        ), f"Failed to fetch favorite properties: {response.text}"
    ...
    async def test_get_favorite_property_by_id(self, client: AsyncClient):
        ...
        assert (
            response.status_code == 200
        ), f"Failed to get favorite property by ID: {response.text}"
    ...
    async def test_filter_favorites(self, client: AsyncClient):
        ...
        response = await client.get(
            "/favorite-properties/", params={"user_id": user_id}
        )
        assert (
            response.status_code == 200
        ), f"Failed to filter favorites by user_id: {response.text}"
        data = response.json()["data"]
        assert all(
            fav["user_id"] == user_id for fav in data
        ), "Filtering by user_id failed"

        # Filter by property_unit_assoc_id
        property_id = self.property_ids[1]
        response = await client.get(
            "/favorite-properties/", params={"property_unit_assoc_id": property_id}
        )
        assert (
            response.status_code == 200
        ), f"Failed to filter favorites by property_unit_assoc_id: {response.text}"
        data = response.json()["data"]
        assert all(
            fav["property_unit_assoc_id"] == property_id for fav in data
    ...
    async def test_delete_favorite_property(self, client: AsyncClient):
        ...
        ), f"Favorite property deletion failed: {response.text}"

        # Verify that the favorite property is deleted
        response = await client.get(f"/favorite-properties/{favorite_id}")
        assert (
            response.status_code == 404
        ), f"Failed to get favorite property by ID after deletion: {response.text}"
